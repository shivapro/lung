...
function preload() {
  ...
  reImage=loadImage("trk7r-yg12y.jpg")
  
  yesImg=loadImage("download (1).jpg")
  
  noImg=loadImage("download (4).jpg")
}
...
health50.addImage(health50Img);
 ...
 ht20to50.visible=false;
  ...
  re=createSprite(displayWidth,displayHeight)
  re.addImage(reImage);
  re.visible=false
  reImage.resize(displayWidth,displayHeight/1)
  
  yes=createSprite(displayWidth/3,displayHeight/2,10,10)
   yes.addImage(yesImg);
    yesImg.resize(0,40)
    yes.visible=false;
    no=createSprite(displayWidth/1.5,displayHeight/2,10,10)
  no.addImage(noImg);
    noImg.resize(0,40)
  no.visible=false;
...
text("Super Human",displayWidth/1.5,displayHeight/2.9) 
  ...
  textSize(12);
    textAlign(CENTER)
    text("Do you smoke? If yes then press the yes button and you will be sent for the lung",displayWidth/2,displayHeight/9)
    
    text("test and if you can stop your breath till normal or between  normal and strong ",displayWidth/2,displayHeight/8.2)
    
    text("then your lungs are weak and you need to stop smoking and retest after three ",displayWidth/2,displayHeight/7.4)
    
     text("months and if you press no then very well but please do regular exercises",displayWidth/2,displayHeight/6.8)
    ...
    if(mousePressedOver(no)){
      yes.visible=false;
      no.visible=false;
      text("Great",displayWidth/2,displayHeight/2);
      gameState=START;
      mainScreen(); 
      backButton.visible=false;
    }
